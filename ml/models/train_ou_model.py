...
def load_data():
    ...
    # Binary target: over = 1, under = 0. Ignore pushes.
    mask = (df["ou_over_win"] | df["ou_under_win"])
    df = df[mask].copy()
    y = df["ou_over_win"].astype(int)
...
def main():
    ...
    print(f"OU model AUC: {auc:.3f}, Acc: {acc:.3f}")

    model_path = os.path.join(MODELS_DIR, "ou_model.pkl")
    joblib.dump(pipe, model_path)
    print(f"Saved OU model to {model_path}")
